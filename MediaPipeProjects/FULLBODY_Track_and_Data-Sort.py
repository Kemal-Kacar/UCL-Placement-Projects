...
FB_FrameFiles = glob.glob("FULLBODY_Data/FB_Frames/*")
for jsF in FB_FrameFiles:
    os.remove(jsF)
FB_Landmarks = glob.glob("FULLBODY_Data/FB_Landmarks/*")
for csvF in FB_Landmarks:
    os.remove(csvF)
FB_cords_tracker = "FULLBODY_Data/FB_cords_tracker.csv"
FB_sorted_landmarks = "FULLBODY_Data/FB_sorted_landmark.csv"
...
with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose:
    while cap.isOpened():
        ...
        if frameCounter < int(cap.get(cv2.CAP_PROP_FRAME_COUNT)):
            filepath = Path("FULLBODY_Data/FB_Frames/frame" + str(frameCounter) + ".json")
...
with open(Path(FB_cords_tracker), "w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(head for head in header)

# open the json frames on the background.
for frames in range(frame_number):
    with open(Path("FULLBODY_Data/FB_Frames/frame" + str(frame_counter) + ".json", "r")) as f:
        data = json.load(f)
        frame_counter += 1
    ...
    while numLandmark < 33:
        for landmark in data["0"][str(numLandmark)], :
            ...
            with open(Path(FB_cords_tracker, "a", newline="")) as cordF:
...
with open(Path(FB_cords_tracker)) as data:
    df = pd.read_csv(data)
    sort = df.sort_values(["landmark"])
    sort.to_csv(Path("FULLBODY_Data/FB_sorted_landmark.csv"), index=False)

sig_list = []
# opens the sorted file and extracts the landmark and distance information.
curLM = 0
with open(Path(FB_sorted_landmarks)) as tbPlot:
    df = pd.read_csv(tbPlot, usecols=["landmark", "distance"])
    # for each landmark creates a CSV file.
    while curLM < 33:
        lm = df.query("landmark == "+str(curLM))
        lmFilePath = Path("FULLBODY_Data/FB_Landmark_CSVs/Landmark" + str(curLM) + ".csv")
...
fileLM = 0
while fileLM < 33:
    with open(Path("FULLBODY_Data/FB_Landmark_CSVs/Landmark"+str(fileLM)+".csv")) as f:
    ...
    with open(Path("FULLBODY_Data/FB_Landmark_CSVs/Landmark"+str(fileLM)+".csv")) as g:
        sigDF = pd.read_csv(g)
        ...
        sigDF.to_csv(Path("FULLBODY_Data/FB_Landmarks/FB_Landmark"+str(fileLM)+".csv"), index=False)
        fileLM += 1
